...
#import "JTMainMessageModel.h"

NS_ASSUME_NONNULL_BEGIN

@class JTConsultationItemModel, JTMutuallyBindingInfoModel, JTMessage;
 ...
 * @param remainingSeconds  会话/排队剩余时间（单位：秒），-1 表示不限时或服务端未下发
 */
typedef void (^JTOnGetConsultationListCallback)(NSArray<JTConsultationItemModel *> *consultationList, NSString *welcomeSentence, int remainingSeconds);
 ...
 * @param error        失败时的错误对象，成功时为 nil
 */
typedef void (^JTOnBindingCallback)(JTMutuallyBindingInfoModel * _Nullable bindingInfo, NSError * _Nullable error);
 ...
 *
 * @param isSocketConnected 当前 WebSocket 状态枚举
 * @param message           描述当前状态的文案，可用于日志或 UI 提示
 */
typedef void (^JTOnWebSocketStateCallback)(JTWebSocketState isSocketConnected, NSString *message);
